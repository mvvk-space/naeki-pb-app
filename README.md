...
| **Rewards** | app | **Points + tiers + offers inbox** — 1 pt / 10฿ (2× Thursdays, ×tier multipliers), Kome→Sake→Maguro→Ikura tiers with perks, weekly brand deals + affinity offers computed from on-device order history ("because you order it"), points redeem 1:1 into wallet |
| **Wallet** | app | **Stored value, Starbucks-style** — TrueMoney top-up (+฿ chips), gift cards (buy from balance, send code, redeem), pays automatically at checkout, Add to Apple Pay / Google Wallet surfaces (honest demo labels) |
